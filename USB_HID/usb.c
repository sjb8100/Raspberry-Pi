...
#include <stdint.h>				// Needed for uint8_t, uint32_t
#include <string.h>				// Needed for memset
#include <wchar.h>				// Needed for UTF for unicode descriptor support
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) ChannelInterrupts {
	union {
		struct {
		...
		volatile uint32_t RawInterrupt;
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) FifoSize {
	union {
		struct {
			volatile unsigned StartAddress : 16; // @0
			volatile unsigned Depth : 16;		 // @16
		};
		volatile uint32_t RawFifoSize;
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) CoreOtgControl {
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) CoreOtgInterrupt {
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) CoreAhb {
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) UsbControl {
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) CoreInterrupts {
	union {
		struct {
		...
		volatile uint32_t RawInterrupt;					// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) NonPeriodicFifoStatus {
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) CoreNonPeriodicInfo {
	volatile struct FifoSize Size;									// +0x28
	volatile const struct NonPeriodicFifoStatus Status;				// Read Only +0x2c
};

/*--------------------------------------------------------------------------}
{						 USB CORE HARDWARE STRUCTURE					    }
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) CoreHardware {
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) CorePeriodicInfo {
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) HostConfig {
	union {
		struct {
		...
		volatile uint32_t RawConfig;					// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) HostFrameInterval {
	union {
		struct {
		...
		volatile uint32_t RawFrameInterval;				// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) HostFrameControl {
	union {
		struct {
			volatile unsigned FrameNumber : 16;			// @0
			volatile unsigned FrameRemaining : 16;		// @16
		};
		volatile uint32_t RawFrameControl;				// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) HostFifoStatus {
	union {
		struct {
			volatile unsigned SpaceAvailable : 16;		// @0
			volatile unsigned QueueSpaceAvailable : 8;	// @16
		...
		volatile uint32_t RawFifoStatus;				// Union to access all 32 bits as a uint32_t
	};
};
...
struct __attribute__((__packed__)) HostPort {
	union {
		struct {
		...
		volatile uint32_t RawPort;						// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) HostChannelCharacteristic {
	union {
		struct {
		...
		volatile uint32_t RawCharacteristic;			// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) HostChannelSplitControl {
	union {
		struct {
		...
		volatile uint32_t RawSplitControl;				// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) HostTransferSize {
	union {
		struct {
		...
		volatile uint32_t RawTransferSize;				// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) HostChannel {
	volatile struct HostChannelCharacteristic Characteristic;	// +0x0
	volatile struct HostChannelSplitControl SplitCtrl;			// +0x4
	volatile struct ChannelInterrupts Interrupt;				// +0x8
	volatile struct ChannelInterrupts InterruptMask;			// +0xc
	volatile struct HostTransferSize TransferSize;				// +0x10
	volatile uint32_t  DmaAddr;									// +0x14
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) PowerReg {
	union {
		struct {
		...
		volatile uint32_t RawPower;						// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
struct __attribute__((__packed__)) UsbSendControl {
	union {
		...
		uint32_t RawUsbSendContol;						// Union to access all 32 bits as a uint32_t
	};
};
...
{--------------------------------------------------------------------------*/
#define DWC_CORE_OTGCONTROL			((volatile struct CoreOtgControl*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x00))
#define DWC_CORE_OTGINTERRUPT		((volatile struct CoreOtgInterrupt*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x04))
#define DWC_CORE_AHB				((volatile struct CoreAhb*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x08))
#define DWC_CORE_CONTROL			((volatile struct UsbControl*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x0C))
#define DWC_CORE_RESET				((volatile struct CoreReset*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x10))
#define DWC_CORE_INTERRUPT			((volatile struct CoreInterrupts*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x14))
#define DWC_CORE_INTERRUPTMASK		((volatile struct CoreInterrupts*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x18))
#define DWC_CORE_RECEIVESIZE		((volatile uint32_t*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x24))
#define DWC_CORE_NONPERIODICFIFO	((volatile struct CoreNonPeriodicInfo*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x28))
#define DWC_CORE_USERID				((volatile uint32_t*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x3C))
#define DWC_CORE_VENDORID			((volatile const uint32_t*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x40))
#define DWC_CORE_HARDWARE			((volatile const struct CoreHardware*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x44))
#define DWC_CORE_PERIODICINFO		((volatile struct CorePeriodicInfo*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x100))

/*--------------------------------------------------------------------------}
{					DWC USB HOST REGISTER POINTERS						    }
{--------------------------------------------------------------------------*/
#define DWC_HOST_CONFIG				((volatile struct HostConfig*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x400))
#define DWC_HOST_FRAMEINTERVAL		((volatile struct HostFrameInterval*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x404))
#define DWC_HOST_FRAMECONTROL		((volatile struct HostFrameControl*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x408))
#define DWC_HOST_FIFOSTATUS			((volatile struct HostFifoStatus*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x410))
#define DWC_HOST_INTERRUPT			((volatile uint32_t*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x414))
#define DWC_HOST_INTERRUPTMASK		((volatile uint32_t*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x418))
#define DWC_HOST_FRAMELIST			((volatile uint32_t*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x41C))
#define DWC_HOST_PORT				((volatile struct HostPort*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x440))
#define DWC_HOST_CHANNEL			((volatile struct HostChannel*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0x500))

/*--------------------------------------------------------------------------}
{					DWC POWER AND CLOCK REGISTER POINTER				    }
{--------------------------------------------------------------------------*/
#define DWC_POWER_AND_CLOCK			((struct PowerReg*)(RPi_IO_Base_Addr + USB_CORE_OFFSET + 0xE00))
...
/* TO CHECK THEM AT COMPILE TIME. IT'S POINTLESS IF THE SIZES ARE WRONG.   */
/*-------------------------------------------------------------------------*/
...
RESULT HcdProcessRootHubMessage (uint8_t* buffer, uint32_t bufferLength, struct UsbDeviceRequest *request, uint32_t *bytesTransferred)
{
	...
	case GetStatus:
		...
		case bmREQ_HUB_STATUS  /*0xa0*/:							// We are a hub class so we need a standard hub class get status return
			replyBuf.replyHub.RawHubFullStatus = 0;					// Zero all the status bits
		...
		case bmREQ_PORT_STATUS /* 0xa3 */:							// PORT request .. Remember we have 1 port which is the actual physical hardware
			if (request->Index == 1) {								// Remember we have only one port so any other port is an error
				...
				replyBuf.replyPort.RawHubPortFullStatus = 0;		// Zero all the status bits
	...
	case ClearFeature:
		...
		case  bmREQ_PORT_FEATURE /*0x23*/:							// Port clear feature request
			if (request->Index == 1) {								// Remember we have only one port so any other port is an error
				...
				case FeatureEnable:
					tempPort = *DWC_HOST_PORT;						// Read the host port
					tempPort.RawPort &= HOSTPORTMASK;				// Cleave off all the triggers
					...
					*DWC_HOST_PORT = tempPort;						// Write the value back
					break;
				case FeatureSuspend:
					DWC_POWER_AND_CLOCK->RawPower = 0;
			...
			if (request->Index == 1) {								// Remember we have only one port so any other port is an error
				...
				case FeatureReset:			
					...
					*DWC_POWER_AND_CLOCK = tempPower;				// Write back to register
					RPI_WaitMicroSeconds(10000);					// Small delay
					DWC_POWER_AND_CLOCK->RawPower = 0;				// Now clear everything
					...
					*DWC_HOST_PORT = tempPort;						// Write the value back
					RPI_WaitMicroSeconds(60000);
					tempPort = *DWC_HOST_PORT;						// Read the host port
					tempPort.RawPort &= HOSTPORTMASK;				// Cleave off all the triggers
					tempPort.Reset = false;							// Clear bit we want
					*DWC_HOST_PORT = tempPort;						// Write the value back
					LOG("Reset physical port .. rootHub %i\n", RootHubDeviceNumber);
					break;
				case FeaturePower:
...
RESULT PowerOnUsb(void) {
	...
	bool result = RPI_MailtagExchange8(&result1, &result2,
		RPI_FIRMWARE_SET_POWER_STATE, 0x03, 0x01);					// Mailbox tag command, device 3 = USB, 1 = on
	if ((result) && (result1 = 3) && (result2 = 1)) return OK;		// Turn on successful
...
RESULT PowerOffUsb(void) {
	uint32_t result1, result2;
	bool result = RPI_MailtagExchange8(&result1, &result2,
		RPI_FIRMWARE_SET_POWER_STATE, 0x03, 0x00);					// Mailbox tag command, device 3 = USB, 0 = off
	if ((result) && (result1 = 3) && (result2 = 0)) return OK;		// Turn off successful
	return ErrorDevice;												// Failed to turn on
}
...
RESULT HCDReset(void) {
	...
	original_tick = RPI_GetArmTickCount();							// Hold original tickcount
	do {
		if (RPI_TickDifference(original_tick, RPI_GetArmTickCount())> 100000) {
...
RESULT HCDTransmitFifoFlush(enum CoreFifoFlush fifo) {
	...
	original_tick = RPI_GetArmTickCount();							// Hold original tick count
	do {
		if (RPI_TickDifference(original_tick, RPI_GetArmTickCount())> 100000) {
			return ErrorTimeout;									// Return timeout error
		}
...
RESULT HCDStart (void) {
	...
	*DWC_CORE_CONTROL = coreUsb;									// Write control register
	...
	if (!PhyInitialised) {											// If physical interface hasn't been initialized
		...
		*DWC_CORE_CONTROL = coreUsb;								// Write control register
		HCDReset();													// You need to do a soft reset to make those settings happen
	}
	...
	*DWC_CORE_CONTROL = coreUsb;									// Write control register 
	...
	DWC_POWER_AND_CLOCK->RawPower = 0;								// Release any power or clock halts given the bit names 
	...
	*DWC_CORE_OTGCONTROL = tempOtgControl;							// Write the Otg register
	...
	if (!DWC_HOST_CONFIG->EnableDmaDescriptor) {
		...
		for (int channel = 0; channel < DWC_CORE_HARDWARE->HostChannelCount; channel++) {
			...
			uint64_t original_tick;
			original_tick = RPI_GetArmTickCount();					// Hold original timertick
			do {
				if (RPI_TickDifference(original_tick, RPI_GetArmTickCount()) > 0x100000) {
	...
	if (!tempPort.Power) {
		...
		tempPort.RawPort &= HOSTPORTMASK;							// Cleave off all the temp bits	
	...
	*DWC_HOST_PORT = tempPort;										// Write value to port
	RPI_WaitMicroSeconds(60000);									// 60ms delay
	tempPort = *DWC_HOST_PORT;										// Fetch host port 
	tempPort.RawPort &= HOSTPORTMASK;								// Cleave off all the temp bits	
...
RESULT HCDWaitOnTransmissionResult(uint32_t timeout, uint8_t channel, struct ChannelInterrupts *IntFlags) {
	...
	uint64_t original_tick = RPI_GetArmTickCount();					// Hold original tick count
	do {
		RPI_WaitMicroSeconds(100);
		if (RPI_TickDifference(original_tick, RPI_GetArmTickCount()) > timeout) {
...
RESULT HCDChannelTransfer(const struct UsbPipe pipe, const struct UsbPipeControl pipectrl, uint8_t* buffer, uint32_t bufferLength, enum PacketId packetId) {
	...
	DWC_HOST_CHANNEL[pipectrl.Channel].Interrupt.RawInterrupt = 0xFFFFFFFF;// Clear all interrupts
	DWC_HOST_CHANNEL[pipectrl.Channel].InterruptMask.RawInterrupt = 0x0;   // Clear all interrupt masks
	...
	do {
		...
		DWC_HOST_CHANNEL[pipectrl.Channel].Interrupt.RawInterrupt = 0xFFFFFFFF;
		DWC_HOST_CHANNEL[pipectrl.Channel].InterruptMask.RawInterrupt = 0x0;
		...
		if (((uint32_t)&buffer[offset] & 3) != 0)
			LOG("HCD: Transfer buffer %#8lx is not DWORD aligned. Ignored, but dangerous.\n", (uint32_t)&buffer[offset]);
		// C gets a little bit quirky because I have deferenced using the array of the structure .. help C out 
		*(uint32_t*)&DWC_HOST_CHANNEL[pipectrl.Channel].DmaAddr = RPI_ARMaddressToBUSaddress(&buffer[offset]);
		...
		if (result) LOG("Result: %i Action: 0x%08lx tempInt: 0x%08lx tempSplit: 0x%08lx Bytes sent: %i\n",
			result, sendCtrl.RawUsbSendContol, tempInt.RawInterrupt, 
			tempSplit.RawSplitControl, result ? 0 : DWC_HOST_CHANNEL[pipectrl.Channel].TransferSize.TransferSize);
		...
		while (sendCtrl.ActionResendSplit) {						// Decision was made to resend split
			...
			if (sendCtrl.LongerDelay) RPI_WaitMicroSeconds(10000);	// Not yet response slower delay
				else  RPI_WaitMicroSeconds(2500);					// Small delay between split resends
		}
								...
								uint32_t length)					// The size of that buffer
{
	...
	uint16_t langIds[128] __attribute__((aligned(4))) = { 0 };		// aligned for DMA transfer a descriptor is max 256 bytes
	...
	result = HCDGetFullDescriptor(pipe, String, 0, 0, &langIds, 2, 
		bmREQ_GET_DEVICE_DESCRIPTOR, &transfer);					// Get language support header
	...
	result = HCDGetFullDescriptor(pipe, String, 0, 0, &langIds,
		langIds[0] & 0xFF, bmREQ_GET_DEVICE_DESCRIPTOR, &transfer);	// Get all language support pair data
...
RESULT HCDInitialise(void) {
	...
	if ((VendorId & 0xfffff000) != 0x4f542000) {					// 'OT'2 
		LOG("HCD: Hardware: %c%c%lx.%lx%lx%lx (BCM%.5lx). Driver incompatible. Expected OT2.xxx (BCM2708x).\n",
			(char)((VendorId >> 24) & 0xff), (char)((VendorId >> 16) & 0xff),
			(VendorId >> 12) & 0xf, (VendorId >> 8) & 0xf,
			(VendorId >> 4) & 0xf, (VendorId >> 0) & 0xf,
			(UserId >> 12) & 0xFFFFF);
		return ErrorIncompatible;
	} else {
		LOG("HCD: Hardware: %c%c%lx.%lx%lx%lx (BCM%.5lx).\n",
			(char)((VendorId >> 24) & 0xff), (char)((VendorId >> 16) & 0xff),
			(VendorId >> 12) & 0xf, (VendorId >> 8) & 0xf,
			(VendorId >> 4) & 0xf, (VendorId >> 0) & 0xf,
			(UserId >> 12) & 0xFFFFF);
	...
	*DWC_CORE_AHB = tempAhb;										// Write temp back to AHB register
	DWC_CORE_INTERRUPTMASK->RawInterrupt = 0;						// Clear all interrupt masks
...
RESULT HubPortReset(struct UsbDevice *device, uint8_t port) {
	...
	for (retry = 0; retry < 3; retry++) {
		...
		do {
			...
			if ((result = HCDReadHubPortStatus(device->Pipe0, port + 1, &portStatus.RawHubPortFullStatus)) != OK) {
...
RESULT EnumerateHID (const struct UsbPipe pipe, struct UsbDevice *device) {
	...
	for (int i = 0; i < device->HidPayload->MaxHID; i++) {
		...
		LOG("HID details: Version: %4lx, Language: %i Descriptions: %i, Type: %i, Protocol: %i, NumInterface: %i\n",
			(uint32_t)Hi << 8 | Lo,
...
RESULT EnumerateHub (struct UsbDevice *device) {
	...
	result = HCDGetDescriptor(device->Pipe0, Hub, 0, 0,
		...
		bmREQ_GET_HUB_DESCRIPTOR, &transfer);						// Fetch the HUB descriptor and hold in the hub payload, we use it a bit so saves USB bus
	...
	if ((result = HCDReadHubPortStatus(device->Pipe0, 0, &status.RawHubFullStatus)) != OK) // Gateway node status
	{
	...
	RPI_WaitMicroSeconds(data->Descriptor.PowerGoodDelay * 2000);	// Every hub has a different power stability delay
...
RESULT EnumerateDevice (struct UsbDevice *device, struct UsbDevice* ParentHub, uint8_t PortNum) {
	...
	struct UsbDeviceDescriptor desc __attribute__((aligned(4)));	// Device descriptor DMA aligned
	...
	RPI_WaitMicroSeconds(10000);									// Allows time for address to propagate.
	...
	result = HCDGetDescriptor(device->Pipe0, Configuration, 0, 0,
		...
		&transfer);													// Read the config descriptor 	
	...
	if ((result != OK) || (transfer != configDesc.TotalLength)) {	// Check if anything went wrong
		LOG("HCD: Failed to read configuration descriptor for device %i, %lu bytes read, Error: %i.\n",
			device->Pipe0.Number, transfer, result);				// Log error
						 ...
						 uint32_t *bytesTransferred)				// Value at pointer will be updated with bytes transfered to/from buffer (NULL to ignore)								
{
	...
	if (result != OK) {												// RESULT in error
		LOG("HCD: Failed to get descriptor %#x:%#x recepient: %#x for device:%i. RESULT %#x.\n",
...
/*-HCDGetFullDescriptor -----------------------------------------------------
 Unlike the normal single descriptor read above this call pulls the amount of
 data requested. It is most used to pull index 0 descriptors.
 24Feb17 LdB
 --------------------------------------------------------------------------*/
RESULT HCDGetFullDescriptor (const struct UsbPipe pipe,				// Pipe structure to send message thru (really just uint32_t) 
							 enum DescriptorType type,				// The type of descriptor
							 uint8_t index,							// The index of the type descriptor
							 uint16_t langId,						// The language id
							 void* buffer,							// Buffer to recieve descriptor
							 uint32_t length,						// Maximum size of provided buffer
							 uint8_t recipient,						// Recipient flags			
							 uint32_t *bytesTransferred)			// Value at pointer will be updated with bytes transfered to/from buffer (NULL to ignore)								
{
	RESULT result;
	...
	result = HCDSumbitControlMessage(
		...
		buffer,														// Buffer pointer passed in as is
		length,														// Length as requested
		&(struct UsbDeviceRequest) {								// We will build a request structure
			.Request = GetDescriptor,								// We want a descriptor obviously
			.Type = recipient,										// Recipient is a flag bmREQ_GET_DEVICE_DESCRIPTOR, bmREQ_GET_HUB_DESCRIPTOR etc
			.Value = (uint16_t)type << 8 | index,					// Type and the index get compacted as the value
			.Index = langId,										// Language ID is the index
			.Length = length,										// Duplicate the length
		},
						  ...
						  uint16_t Length)							// Maxium length of the buffer 
{
	...
	result = HCDGetFullDescriptor(device->Pipe0, HidReport, 0,
		device->HidPayload->HIDInterface[hidIndex],					// Index number of HID index
		&buf[0], sizeToRead, 0x81, &transfer);						// Read the HID report descriptor 	
