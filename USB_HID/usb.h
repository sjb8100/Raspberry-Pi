...
enum UsbDeviceStatus {
	USB_STATUS_ATTACHED = 0,
	USB_STATUS_POWERED = 1,
	USB_STATUS_DEFAULT = 2,
	USB_STATUS_ADDRESSED = 3,
	USB_STATUS_CONFIGURED = 4,
};
...
struct __attribute__((__packed__)) UsbConfigurationDescriptor {
	...
	} __attribute__((__packed__)) Attributes;						// +0x7 Configuration characteristics
...
struct __attribute__((__packed__)) UsbEndpointDescriptor {
	...
	} __attribute__((__packed__)) EndpointAddress;					// +0x2  Endpoint address. Bit 7 indicates direction (0=OUT, 1=IN).
	struct {
	...
	} __attribute__((__packed__)) Attributes;						// +0x3 Endpoint transfer type
	struct {
	...
	} __attribute__((__packed__)) Packet;							// +0x4 Maximum packet size.
...
struct __attribute__((__packed__)) HubDescriptor {
	...
	} __attribute__((__packed__)) DeviceRemovable;					// +0x7
...
struct __attribute__((__packed__)) HubFullStatus {
	union {
		...
		uint32_t RawHubFullStatus;									// Both status joined as one raw 32 bits
	};
};
...
struct HubPortFullStatus {
	union {
		...
		uint32_t RawHubPortFullStatus;								// Both status joined as one raw 32 bits
	};
} __attribute__((__packed__));
...
{--------------------------------------------------------------------------*/
struct UsbConfigControl {
	unsigned ConfigIndex : 8;							// @0 Current set config index
	unsigned ConfigStringIndex : 8;						// @8 Current config string index
	enum UsbDeviceStatus Status : 8;					// @16 Device software enumeration status .. USB_ATTACHED, USB_POWERED, USB_ADDRESSED, etc
...
/*-HCDGetFullDescriptor -----------------------------------------------------
 ...
 data requested. It is most used to pull index 0 descriptors, which are long
...
RESULT HCDSetDescriptor(const struct UsbPipe pipe,					// Pipe structure to send message thru (really just uint32_t) 
	enum DescriptorType type,					// The type of descriptor
	uint8_t index,								// The index of the type descriptor
	uint16_t langId,							// The language id
	void* buffer,								// Buffer with descriptorto write
	uint32_t length,							// Maximum length of descriptor
	uint8_t recipient);							// Recipient flags		
/*--------------------------------------------------------------------------}
